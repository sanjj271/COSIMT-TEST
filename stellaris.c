...
static void LM3S811EVB_create_remoteport(MachineState *machine,
                                   MemoryRegion *system_memory)
{
    ...
    object_property_set_int(rpm_obj, "map-offset", memmap[M3_COSIM].base, &error_fatal);
    object_property_set_int(rpm_obj, "map-size", memmap[M3_COSIM].size, &error_fatal);
...
//    object_property_set_int(rpms_obj, 0, "rp-chan0", &error_fatal);
    ...
    memory_region_add_subregion(system_memory, memmap[M3_COSIM].base,
...
static void lm3s811evb_init(MachineState *machine)
{
    ...
    if (machine_path) {
        LM3S811EVB_create_remoteport(machine, system_memory);
    }
}
